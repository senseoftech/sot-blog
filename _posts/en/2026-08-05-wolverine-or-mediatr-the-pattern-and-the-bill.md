...
Three honest options, one candidate worth the detour (Wolverine), and a golden reminder about what [your slices]({{ site.baseurl }}/2026/07/22/vibe-engineering-vertical-slice-architecture/) owe to the pattern — not the package. You'll see: it's not rocket science.
...
Re-read the structure of a [vertical slice]({{ site.baseurl }}/2026/07/22/vibe-engineering-vertical-slice-architecture/): a thin endpoint, a message, a testable handler. That shape survives MediatR, Wolverine, the homemade dispatch — **because the pattern structures, not the package.** That's exactly why the [base prompt]({{ site.baseurl }}/2026/07/12/vibe-project-dotnet-foundations/) mandates "an endpoint delegates to a handler" rather than a library name. Well-laid foundations make tollbooths negotiable.
