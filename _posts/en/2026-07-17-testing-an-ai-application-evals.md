...
Red. You rerun: red again, but **differently** — the model rephrased. Same input, different output, [that's its nature]({{ site.baseurl }}/2026/07/16/why-ai-hallucinates/). So how do you test a component that never answers the same way twice? Welcome to the world of **evals**. You'll see: it's not rocket science.
...
**The word of honesty**, obviously: the judge is an LLM — it can be wrong, [like the others]({{ site.baseurl }}/2026/07/16/why-ai-hallucinates/). So you **calibrate** it: on a sample, a human grades too, you compare, and you adjust the rubric until judge and human agree. A grader gets trained before grading alone.
