...
In [the RAG and embeddings article]({{ site.baseurl }}/2026/07/18/rag-embeddings-explained-simply/), we understood the theory: the map of ideas, the library sorted by meaning, the librarian laying three excerpts on the expert's desk. Today we actually hire him: **a complete RAG in C#**, with the official `Microsoft.Extensions.AI` abstractions — the same kind of standard interface as `ILogger`, but for AI.
...
The two classic mistakes, [already named]({{ site.baseurl }}/2026/07/18/rag-embeddings-explained-simply/): too big (the chunk drowns the information in noise), too small (the chunk loses its context). 1,000 to 2,000 characters per chunk is a healthy starting point — *a starting point*: you tune it by measuring (step 5).
...
Note the `Source` field: the originating file and section travel **with** each chunk. That's what will enable citations — non-negotiable since [the hallucinations article]({{ site.baseurl }}/2026/07/19/why-ai-hallucinates/).
...
The moment [the evals article]({{ site.baseurl }}/2026/07/20/testing-an-ai-application-evals/) meets this one: RAG quality is tuned **by measuring the search**, not by switching models. The minimal golden dataset: twenty real questions, and for each one, the chunk that *should* come up:
