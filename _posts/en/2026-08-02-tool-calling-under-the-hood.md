...
In .NET, that's the response-format parameter of the SDKs (and `GetResponseAsync<T>` in `Microsoft.Extensions.AI`, which deserializes straight into your type). The rule: **the shape by contract, the substance by [evals]({{ site.baseurl }}/2026/07/20/testing-an-ai-application-evals/)** — the schema guarantees valid JSON, not true JSON.
