...
Add up what the series has established: an agent [calls tools in a loop]({{ site.baseurl }}/2026/07/29/tool-calling-under-the-hood/), [resumes on a checkpoint]({{ site.baseurl }}/2026/07/27/agent-framework-workflow-hands-on/), [delegates to subagents]({{ site.baseurl }}/2026/07/25/copilot-subagents-splitting-the-work/), and is [never deterministic]({{ site.baseurl }}/2026/08/10/sampling-and-constrained-decoding/). You don't have an "AI script": you have a **distributed, asynchronous, partially failing system** — with, as a bonus, a component that improvises.
...
The crucial, counterintuitive architecture point: **responsibility for idempotency belongs to the tool, not the agent.** You don't *trust* the model not to replay — [it's a gullible intern]({{ site.baseurl }}/2026/08/06/prompt-injection-defense-in-depth/) — you make the replay **harmless** at the MCP server level. Design your sensitive tools as idempotent endpoints, exactly like a good payment API.
...
Reserve stock → charge the card → ship. If shipping fails, you don't "rollback" — you **compensate**: refund the card, release the stock. Compensation doesn't erase history (the charge existed), it **corrects it with an inverse action**. It's the [workflows' RequestPort/checkpoint]({{ site.baseurl }}/2026/07/27/agent-framework-workflow-hands-on/) taking on its full meaning: every crossed step is a resume point *and* a possible compensation point.
...
This is where [OTel observability]({{ site.baseurl }}/2026/07/31/observing-your-agents-opentelemetry/) becomes vital: traces are the raw material of those SLOs, and [last night's incident becoming tomorrow's eval]({{ site.baseurl }}/2026/07/17/testing-an-ai-application-evals/) closes the quality loop. A system you don't observe is a system whose failures you discover through your customers.
