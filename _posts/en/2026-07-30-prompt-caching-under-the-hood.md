...
2. **The tool catalog** ([yesterday's JSON Schemas]({{ site.baseurl }}/2026/07/29/tool-calling-under-the-hood/)) — never moves mid-session.
...
- **And compaction!** The link with [the context window article]({{ site.baseurl }}/2026/07/23/context-window-compress-forget/): summarizing history **rewrites the prefix** — and thus invalidates the cache. It's a real trade-off: compaction shrinks the context durably, but the next turn repays everything at full price. One more reason to compact *at the right moments* (between tasks), not in the middle of a tool loop.
