...
Les deux erreurs classiques, [déjà nommées]({{ site.baseurl }}/fr/2026/07/15/rag-embeddings-expliques-simplement/) : trop gros (le passage noie l'info dans du bruit), trop petit (le passage perd son contexte). 1 000 à 2 000 caractères par morceau est un point de départ sain — *un point de départ* : on ajuste en mesurant (étape 5).
...
Notez le champ `Source` : le fichier et la section d'origine voyagent **avec** chaque morceau. C'est lui qui permettra les citations — non négociable depuis [l'article hallucinations]({{ site.baseurl }}/fr/2026/07/16/pourquoi-l-ia-hallucine/).
...
Le moment où [l'article évals]({{ site.baseurl }}/fr/2026/07/17/tester-une-application-ia-les-evals/) rejoint celui-ci : la qualité d'un RAG se règle **en mesurant la recherche**, pas en changeant de modèle. Le golden dataset minimal : vingt questions réelles, et pour chacune, le morceau qui *devrait* remonter :
