...
Dans [le dernier épisode de la série CLI]({{ site.baseurl }}/fr/2026/07/21/copilot-cli-4-deleguer-et-automatiser/), un caractère magique — `&` — envoyait une tâche « au cloud ». Il est temps de rencontrer celui qui la reçoit : le **Copilot coding agent**, le troisième visage de Copilot. L'IDE assiste, le CLI habite votre terminal… et le coding agent, lui, **travaille sur GitHub.com pendant que vous faites autre chose**.
...
Vous avez déjà tout appris dans [l'article sous-agents]({{ site.baseurl }}/fr/2026/07/25/copilot-sous-agents-decouper-le-travail/) : le coding agent ne sait que ce que l'issue lui dit — **une issue bien rédigée EST le brief**. La même anatomie s'applique, mot pour mot :
...
Ce qu'il fait bien : les bugs reproductibles, la dette ciblée, les améliorations de couverture, les tâches nettes d'une [tranche verticale]({{ site.baseurl }}/fr/2026/07/22/vibe-engineering-vertical-slice-architecture/) — le périmètre borné, encore lui. Ce qu'on ne lui confie pas : les décisions d'architecture ([elles se prennent avec vous, et se consignent en ADR]({{ site.baseurl }}/fr/2026/07/14/adr-memoire-decisions-architecture/)), le flou (« améliore la perf ») et tout ce dont le critère de succès ne tient pas dans l'issue.
...
- **Les serveurs MCP** — configurables pour l'agent aussi : [votre serveur stock]({{ site.baseurl }}/fr/2026/07/26/serveur-mcp-en-production/), les outils internes… l'agent cloud a droit aux mêmes prises que l'équipe.
...
Autrement dit : le pire scénario réaliste est *une PR médiocre que vous refusez*. C'est le [rayon d'explosion borné]({{ site.baseurl }}/fr/2026/07/22/vibe-engineering-vertical-slice-architecture/), version organisationnelle. Vos réflexes de [l'article sécurité]({{ site.baseurl }}/fr/2026/07/10/securiser-github-copilot/) restent de mise — la revue de PR est *le* point de contrôle, traitez-la comme telle.
