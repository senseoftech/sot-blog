...
- La « mémoire » n'est pas de l'apprentissage : le modèle ne devient pas plus intelligent, il est **mieux briefé** (la nuance [fine-tuning vs contexte]({{ site.baseurl }}/fr/2026/07/15/rag-embeddings-expliques-simplement/), encore elle).
...
L'étage qui nous concerne le plus, nous développeurs — vous l'avez croisé dans [l'épisode 2 de la série CLI]({{ site.baseurl }}/fr/2026/07/19/copilot-cli-2-le-quotidien/) :
...
- **Le trop-plein** : mémoriser trop, c'est ressusciter [la dilution d'hier]({{ site.baseurl }}/fr/2026/07/23/fenetre-de-contexte-compresser-oublier/). Une bonne mémoire **oublie** — c'est une fonctionnalité, pas un défaut.
