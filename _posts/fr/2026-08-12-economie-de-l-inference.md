...
Le [KV cache]({{ site.baseurl }}/fr/2026/08/08/attention-kv-cache-sous-le-capot/) nous a donné la clé physique ; on la transforme aujourd'hui en économie. VRAM, batching continu, quantization, et le calcul de bascule API vs auto-hébergé. Vous allez voir : c'est pas sorcier.
...
2. **Le KV cache** — coût **variable, par requête et par token**. Souvenez-vous : [chaque token en vol garde ses Keys et Values]({{ site.baseurl }}/fr/2026/08/08/attention-kv-cache-sous-le-capot/) en VRAM. Une conversation de 8 000 tokens réserve donc une tranche de mémoire proportionnelle — *pendant toute sa durée*.

La conséquence financière est contre-intuitive et centrale : **« contexte long » = « mémoire GPU réservée » = « moins de conversations simultanées sur la même carte ».** Le coût réel d'un gros contexte n'est pas seulement le [calcul quadratique]({{ site.baseurl }}/fr/2026/08/08/attention-kv-cache-sous-le-capot/), c'est qu'il **occupe une place** que d'autres utilisateurs ne peuvent plus avoir. Votre facture au token n'est qu'un proxy de ça.
...
- **La latence** — le temps pour *votre* réponse. Décomposée : le **TTFT** (time to first token, dominé par le [prefill quadratique]({{ site.baseurl }}/fr/2026/08/08/attention-kv-cache-sous-le-capot/) de tout votre prompt) puis les **tokens/seconde** du decode.
...
Les méthodes modernes (AWQ, GPTQ) sont **intelligentes** : elles préservent finement les poids les plus sensibles au lieu de tout arrondir bêtement. Le 4 bits fait tenir un 70B sur une seule grosse carte — la différence entre « louable » et « inaccessible ». **Le mot d'honnêteté** : la perte est réelle et **à mesurer sur *votre* tâche** — avec [vos évals]({{ site.baseurl }}/fr/2026/07/17/tester-une-application-ia-les-evals/), pas sur un benchmark générique. Parfois indolore, parfois éliminatoire selon la sensibilité de votre cas.
