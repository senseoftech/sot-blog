...
Dans [l'article sur les tokens]({{ site.baseurl }}/fr/2026/07/05/tokens-llm-envoyes-sortie-cache/), une ligne du tableau de prix intriguait : les *tokens cachés*, facturés une fraction du prix normal. Et dans [la boucle du tool calling]({{ site.baseurl }}/fr/2026/08/02/tool-calling-sous-le-capot/), un détail aurait dû vous alarmer : à chaque aller-retour, **tout le contexte est renvoyé et refacturé**. Un agent qui enchaîne trente appels d'outils relit trente fois les mêmes instructions.
...
2. **Le catalogue d'outils** ([les JSON Schemas d'hier]({{ site.baseurl }}/fr/2026/08/02/tool-calling-sous-le-capot/)) — ne bouge jamais en cours de session.
