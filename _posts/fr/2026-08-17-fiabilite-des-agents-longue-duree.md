...
Additionnez ce que la série a établi : un agent [appelle des outils en boucle]({{ site.baseurl }}/fr/2026/08/02/tool-calling-sous-le-capot/), [reprend sur checkpoint]({{ site.baseurl }}/fr/2026/07/31/agent-framework-workflow-pratique/), [délègue à des sous-agents]({{ site.baseurl }}/fr/2026/07/29/copilot-sous-agents-decouper-le-travail/), et n'est [jamais déterministe]({{ site.baseurl }}/fr/2026/08/15/sampling-decodage-contraint/). Vous n'avez pas un « script IA » : vous avez un **système distribué, asynchrone, partiellement défaillant** — avec, en prime, un composant qui improvise.
...
Le point d'architecture crucial, contre-intuitif : **la responsabilité de l'idempotence appartient à l'outil, pas à l'agent.** On ne *fait pas confiance* au modèle pour ne pas rejouer — [c'est un stagiaire crédule]({{ site.baseurl }}/fr/2026/08/13/prompt-injection-defense-en-profondeur/) — on rend le rejeu **inoffensif** au niveau du serveur MCP. Concevez vos outils sensibles comme des endpoints idempotents, exactement comme une bonne API de paiement.
...
Réserver le stock → débiter la carte → expédier. Si l'expédition échoue, on ne « rollback » pas — on **compense** : rembourser la carte, relâcher le stock. La compensation n'efface pas l'histoire (le débit a existé), elle la **corrige par une action inverse**. C'est le [RequestPort/checkpoint des workflows]({{ site.baseurl }}/fr/2026/07/31/agent-framework-workflow-pratique/) qui prend tout son sens : chaque étape franchie est un point de reprise *et* un point de compensation possible.
...
C'est là que [l'observabilité OTel]({{ site.baseurl }}/fr/2026/08/04/observer-ses-agents-opentelemetry/) devient vitale : les traces sont la matière première de ces SLO, et [l'incident de cette nuit qui devient l'éval de demain]({{ site.baseurl }}/fr/2026/07/20/tester-une-application-ia-les-evals/) boucle la boucle qualité. Un système qu'on n'observe pas est un système dont on découvre les défaillances par les clients.
