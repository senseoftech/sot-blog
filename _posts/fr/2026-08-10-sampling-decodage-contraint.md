...
Niveau 400, épisode 3. Dans [l'article sur les hallucinations]({{ site.baseurl }}/fr/2026/07/16/pourquoi-l-ia-hallucine/), on a croisé la **température** — « le dosage du hasard ». Et dans [le tool calling]({{ site.baseurl }}/fr/2026/07/29/tool-calling-sous-le-capot/), les **structured outputs** — « le contrat qui contraint la génération ». Deux affirmations lâchées sans le mécanisme. Aujourd'hui, on regarde comment un token est *réellement* choisi.
...
Reprenons là où [le KV cache]({{ site.baseurl }}/fr/2026/08/08/attention-kv-cache-sous-le-capot/) s'arrêtait. Après avoir digéré le contexte, le modèle ne produit pas « un mot ». Il produit, pour **chaque token possible du vocabulaire** (des dizaines de milliers), un score brut — un **logit**. Une fonction (softmax) transforme ces logits en **distribution de probabilités** : *« "Paris" 82 %, "la" 6 %, "Lyon" 3 %… »* sur tout le vocabulaire.
...
Conséquence pour l'architecte : **le déterminisme n'est pas une propriété sur laquelle bâtir.** C'est exactement ce qui fonde [l'approche par évals]({{ site.baseurl }}/fr/2026/07/17/tester-une-application-ia-les-evals/) — on gère des *seuils*, pas des sorties identiques — et [l'ingénierie statistique des évals]({{ site.baseurl }}/fr/2026/08/06/prompt-injection-defense-en-profondeur/) que ce blog abordera bientôt. Un `seed` fixe aide en développement ; il ne garantit rien à travers un parc GPU.

## Le décodage contraint : comment le JSON devient valide *par construction*

Reste la promesse du [tool calling]({{ site.baseurl }}/fr/2026/07/29/tool-calling-sous-le-capot/) à honorer : les **structured outputs**. Le mécanisme est d'une élégance limpide et se loge *exactement* à l'étape de sampling.
